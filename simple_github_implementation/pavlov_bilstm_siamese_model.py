...
class BiLSTMSiameseNetwork():
    ...
    def create_model(self) -> Model:
        ...
        if self.triplet_mode:
            ...
            dist = Lambda(self._diff_mult_dist)([lstm_c, lstm_r])                   # get question, answer, difference, and product
            dist = Dense(1, activation='sigmoid', name="score_model")(dist)         # dense layer for a learned loss function which is a combination of the inputs
    ...
    def _diff_mult_dist(self, inputs: List[Tensor]) -> Tensor:
        input1, input2 = inputs         # question and answer
        a = K.abs(input1 - input2)
        b = Multiply()(inputs)
        return K.concatenate([input1, input2, a, b])    # inputs to the learned loss function
